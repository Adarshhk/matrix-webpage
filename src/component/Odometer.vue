<template>
    <div class="">
        <div class="odometer">{{ formattedNumber }}</div>
  ...
  // Initial number
  const number = ref(152356)
  
  // Formatted number with comma separators
  const formattedNumber = ref(number.value.toLocaleString())
  ...
  onMounted(() => {
    timer = setInterval(() => {
      ...
      // Update formatted number with comma separators
